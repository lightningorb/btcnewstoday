<script lang="ts">
  ...
  async function get_note(note_id, author_pk){
    ...
    if (note_id == null || author_pk == null)
      return
    if (note_id == '' || author_pk == '')
      return
    console.log('getting')
    const res = await fetch(API_FQDN+`/api/third_party/nostr_note_text/?note_id=${note_id}&author_pk=${author_pk}`);
  ...
  function add_note(note_id, author_pk, text, username){
    ...
    axios.post('/api/nostr_notes/', body, {headers: headers})
...
<div>
  <Button size="sm" on:click={toggle}><img src='/add-nostr-note.png' style='width: 14px;'/></Button>
  ...
  <Modal body header="Add Nostr Note" isOpen={open} {toggle}>
    ...
    <br/>
    <Button color="danger" on:click={() => add_note(note_id, author_pk, text, username)} size="sm">Add Note</Button>
</FormGroup>
