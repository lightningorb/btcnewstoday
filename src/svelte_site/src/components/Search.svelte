<script>
  import {
    ...
    function onkeypress(x) {
        axios.get(API_FQDN + '/api/meta/search/?term=' + term)
...
{#each res as r}
    ...
    <Card>
      <CardBody>
        <CardSubtitle><a target='_blank' ref='noreferrer' href='https://btcnews.today/{r.date}/{r.id}#{r.id}'>{r.title}</a></CardSubtitle>
        <CardText>
            {@html r.highlights}
        </CardText>
      </CardBody>
