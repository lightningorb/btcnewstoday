<script>
	...
	function _delete() {
		axios
			.post(`${API_FQDN}/api/delete_podcast/${podcast.id}/`)
	...
	function save() {
		...
		axios
			.post(`${API_FQDN}/api/update_podcast/`, body, { headers: headers })
			.then(function (response) {
				console.log(response);
				edit = false;
			})
...
<tr>
	...
	{:else}
		<td
			><Play {podcast} /><a rel="noreferrer" target="_blank" href={podcast.link}>{podcast.outlet}</a
			></td
		>
		<td>
